...
class Ghost(Site):
    ...
    def post_install(self, vars_, dbpasswd=""):
        ...
        with open(os.path.join(self.path,
                               'core/server/models/user.js'), 'r') as f:
            d = f.read()
        d = d.replace('bcryptjs', 'bcrypt')
        with open(os.path.join(self.path,
                               'core/server/models/user.js'), 'w') as f:
            f.write(d)
        ...
        with open(os.path.join(self.path, 'config.example.js'), 'r') as f:
            data = f.read()
        data = data.replace("port: '2368'", "port: '{0}'"
                            .format(self.backend_port))
        data = data.replace('http://my-ghost-blog.com',
                            'http://{0}'+(':'+str(self.port)
                                          if str(self.port) != '80'
                                          else'')
                            .format(self.addr))
        ...
        cfg = {
                ...
                'user': 'ghost',
                'command': 'node {0}'
                .format(os.path.join(self.path, 'index.js')),
                'autostart': 'true',
                'autorestart': 'true',
        ...
        for x in n.servers:
            if x.filter('Location', '/'):
                ...
                for y in x.filter('Location', '/')[0].all():
                    if y.value == 'X-Forwarded-For '\
                            '$proxy_add_x_forwarded_for' or \
