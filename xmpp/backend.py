...
def on_load(app):
    ...
    doms = domains.get()
    for x in doms:
        if not os.path.exists("/etc/prosody/conf.d/{0}.cfg.lua"
                              .format(x.name)):
            add_domain(x, False)
            reload = True
...
def add_domain(domain, reload=True):
    ...
    with open("/etc/prosody/conf.d/{0}.cfg.lua".format(domain.name), "w") as f:
        data = ['VirtualHost "{0}"\n'.format(domain.name)]
...
def remove_domain(domain, reload=True):
    ...
    if os.path.exists("/etc/prosody/conf.d/{0}.cfg.lua".format(domain.name)):
        os.unlink("/etc/prosody/conf.d/{0}.cfg.lua".format(domain.name))
    if reload:
        services.get("prosody").restart()
...
def add_ssl(name, cert, key):
    ...
    try:
        ...
        pass
    return {"type": "app", "id": "xmpp_{0}".format(name), "aid": "xmpp",
            "sid": name, "name": "Chat Server ({0})".format(name)}
